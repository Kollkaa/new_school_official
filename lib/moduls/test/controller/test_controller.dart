  ...
  void startTimer() {
    timer = new Timer.periodic(Duration(seconds: 1), (Timer timer) async {
      if (progress.value <= 0) {
        timer.cancel();
        print(
            test.data['questions'][currentIndexQuestion.value]['question_id']);
        if (indexAnswer.value != "") {
          var response = await Backend().sendQuery(
              question_id: test.data['questions'][currentIndexQuestion.value]
                  ['question_id'],
              answer_id: jsonDecode(indexAnswer.value)['answer_id'],
              id: mainController.profile['id'],
              course_id: homeController.course['kurses'][0]['id']);
          print(response);
